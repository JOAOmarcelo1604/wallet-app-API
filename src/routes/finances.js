...
router.delete("/:id", async (req, res) => {
  try {
    ...
    // Verificação de existência do item de finanças
    const findFinancesText = "SELECT * FROM finances WHERE id=$1";
    const findFinanceValue = [Number(id)];
    const financeItemQuery = await db.query(findFinancesText, findFinanceValue);

    if (!financeItemQuery.rows[0]) {
      return res.status(404).json({ error: "Finance row not found" });
    }

    // Verificação de propriedade do item
    ...
    // Exclusão do item de finanças
    const deleteQuery = "DELETE FROM finances WHERE id=$1 RETURNING *";
    const deleteValues = [Number(id)];
    const deleteResponse = await db.query(deleteQuery, deleteValues);
